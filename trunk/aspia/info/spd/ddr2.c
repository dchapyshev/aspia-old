...
ShowSpdDataForDDR2(BYTE *Spd)
{
    ...
    IoAddHeaderString((SafeStrLen(szText) == 0) ? L"Unknown" : szText, 0);
    ...
    ItemIndex = IoAddValueName(IDS_SPD_DIMM_TYPE, -1);
    if (Spd[0x14] & 0x01)
    ...
    ItemIndex = IoAddValueName(IDS_SPD_ANALYSIS_PROBE, -1);
    if ((Spd[0x15] & 0x40) >> 5)
    ...
    ItemIndex = IoAddValueName(IDS_SPD_TIMING_RTP, -1);
    StringCbPrintf(szText, sizeof(szText), L"%0.2f ns", GetSpdTime(Spd[0x26]));
    IoSetItemText(ItemIndex, 1, szText);

    ItemIndex = IoAddValueName(IDS_SPD_TIMING_WTR, -1);
    StringCbPrintf(szText, sizeof(szText), L"%0.2f ns", GetSpdTime(Spd[0x25]));
    IoSetItemText(ItemIndex, 1, szText);

    ItemIndex = IoAddValueName(IDS_SPD_TIMING_WR, -1);
